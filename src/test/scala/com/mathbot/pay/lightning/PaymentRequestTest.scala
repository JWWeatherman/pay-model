...
class PaymentRequestTest extends AnyFunSuite {
  ...
  test(
    ...
    val pr = LightningPaymentRequest.read(ref)
...
//    assert(pr.amount.isEmpty)
    ...
    assert(LightningPaymentRequest.write(pr.sign(priv)) == ref)
  }
  ...
  test("Please send $3 for a cup of coffee to the same peer, within 1 minute") {
    ...
    val pr = LightningPaymentRequest.read(ref)
    assert(pr.prefix == "lnbc")
    assert(pr.amount === Some(MilliSatoshi(250000000)))
    assert(pr.paymentHash.bytes == hex"0001020304050607080900010203040506070809000102030405060708090102")
    assert(pr.timestamp == 1496314658L)
    assert(pr.nodeId == PublicKey(hex"03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad"))
  ...
  test("Now send $24 for an entire list of things (hashed)") {
    ...
    val pr = LightningPaymentRequest.read(ref)
    assert(pr.prefix == "lnbc")
    assert(pr.amount === Some(MilliSatoshi(2000000000)))
    assert(pr.paymentHash.bytes == hex"0001020304050607080900010203040506070809000102030405060708090102")
    assert(pr.timestamp == 1496314658L)
    assert(pr.nodeId == PublicKey(hex"03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad"))
    assert(
      pr.description == Right(
        Crypto.sha256(
          ByteVector(
            "One piece of chocolate cake, one icecream cone, one pickle, one slice of swiss cheese, one slice of salami, one lollypop, one piece of cherry pie, one sausage, one cupcake, and one slice of watermelon".getBytes
          )
        )
      )
    )
  ...
  test("nonreg") {
    ...
    for (req <- requests) {
      assert(LightningPaymentRequest.write(LightningPaymentRequest.read(req)) == req)
    }
  }
}
