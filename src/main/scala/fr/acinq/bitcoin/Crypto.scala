...
object Crypto {
  ...
  object PrivateKey {
    def apply(data: ByteVector): PrivateKey =
      new PrivateKey(
  ...
  object PublicKey {
    def apply(data: ECPoint): PublicKey =
      new PublicKey(
  ...
  def checkSignatureEncoding(sig: ByteVector, flags: Int): Boolean = {
    ...
    if (sig.isEmpty) true
    else if ((flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRICTENC)) != 0 && !isDERSignature(
               sig
             )) false
  ...
  def checkPubKeyEncoding(
    ...
    if ((flags & ScriptFlags.SCRIPT_VERIFY_WITNESS_PUBKEYTYPE) != 0 && sigVersion == SigVersion.SIGVERSION_WITNESS_V0)
      require(
  ...
  def isDefinedHashtypeSignature(sig: ByteVector): Boolean =
    if (sig.isEmpty)
      false
    else {
   ...
   */
  def publicKeyFromPrivateKey(privateKey: ByteVector) =
    PrivateKey(
      privateKey
    ).publicKey
