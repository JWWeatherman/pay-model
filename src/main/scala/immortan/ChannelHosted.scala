...
abstract class ChannelHosted extends Channel { me =>
  ...
  def doProcess(change: Any): Unit =
    Tuple3(data, change, state) match {
          ...
          val checker = new PreimageCheck {
            override def onComplete(hash2preimage: HashToPreimage): Unit = {
              ...
              val (fulfilled, failed) =
                sentExpired.values.flatten.partition(add => hash2preimage contains add.paymentHash)
              localSuspend(
      ...
      case (hc: HostedCommits, remoteInfo: RemoteNodeInfo, _) if hc.remoteInfo.nodeId == remoteInfo.nodeId =>
