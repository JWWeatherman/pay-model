        ...
        paymentInfoOpt match {
          ...
          case Some(info) =>
            info.description.toSelfPreimage match {
              case _ if info.isIncoming && PaymentStatus.SUCCEEDED == info.status =>
                becomeRevealed(
                  info.preimage,
                  info.description.queryText,
                  adds
                ) // Already revealed, but not finalized
              case Some(selfPreimage) if !info.isIncoming && PaymentStatus.SUCCEEDED == info.status =>
                becomeRevealed(
                  selfPreimage,
                  info.description.queryText,
                  adds
              ...
              case Some(selfPreimage) if !info.isIncoming && info.prExt.isEnough(lastAmountIn) =>
                becomeRevealed(
                  selfPreimage,
                  info.description.queryText,
                  adds
...
class TrampolinePaymentRelayer(val fullTag: FullPaymentTag, cm: ChannelMaster)
  ...
  override def gotFirstPreimage(
    ...
    self doProcess TrampolineRevealed(
  ...
  def validateRelay(
    ...
    firstOpt(adds) collectFirst {
      case pkt if pkt.innerPayload.invoiceFeatures.isDefined && pkt.innerPayload.paymentSecret.isEmpty =>
      ...
      case pkt if adds.map(_.packet.innerPayload.amountToForward).toSet.size != 1 =>
        IncorrectOrUnknownPaymentDetails(
          pkt.add.amountMsat,
          LNParams.blockCount.get
