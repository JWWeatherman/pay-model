...
case class SyncWorkerPHCData(
  ...
  def withNewUpdate(cu: ChannelUpdate): SyncWorkerPHCData = {
    ...
    copy(
      expectedPositions = expectedPositions.updated(cu.shortChannelId, oneLessPosition),
      updates = updates + cu
    )
  }
...
case class SyncWorker(
  ...
  def doProcess(change: Any): Unit =
    (change, data, state) match {
      ...
      case (na: NodeAnnouncement, d1: SyncWorkerGossipData, GOSSIP_SYNC) if Announcements.checkSig(na) =>
...
abstract class SyncMaster(
  ...
  def hasCapacityIssues(update: ChannelUpdate): Boolean =
    update.htlcMaximumMsat.forall(cap => cap < LNParams.syncParams.minCapacity || cap <= update.htlcMinimumMsat)
    ...
    (change, data, state) match {
      ...
      case (
        ...
        if (ranges2.size == maxConnections) {
          ...
          provenShortIds = accum.collect {
            case (shortId, confs) if confs > LNParams.syncParams.acceptThreshold =>
              shortId
          }.toSet
      ...
      case (
        ...
        for (liteUpdate <- workerData.updates)
          confirmedChanUpdates(liteUpdate.core) = confirmedChanUpdates(liteUpdate.core).add(liteUpdate, sync.pair.them)
  ...
  def getPureNormalNetworkData: PureRoutingData = {
    val goodAnnounces = confirmedChanAnnounces.collect {
      case (announce, confirmedByNodes) if confirmedByNodes.size > LNParams.syncParams.acceptThreshold =>
        announce
    }.toSet
    val goodUpdates = confirmedChanUpdates.values.collect {
      case UpdateConifrmState(Some(update), confs) if confs.size > LNParams.syncParams.acceptThreshold =>
        update
    }.toSet
